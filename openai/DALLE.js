const { startTyping, stopTyping, download, extractImgID } = require("../utils/utils.js");
...
async function imageCompletion(prompt, message, number) {
    ...
    startTyping(message);
    openai.createImage(imagePrompt).then(res => {
        ...
        stopTyping();
        message.channel.send("An error occured: " + e.data.response.error.message);
        console.log(e.data.response.error.message);
    })
}
