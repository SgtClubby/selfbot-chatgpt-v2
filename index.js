...
const { clearContextByGuildId, getContextByGuildId } = require("./mongo/mongo.js");
...
client.on('messageCreate', async message => {
    ...
    switch (command) {
        ...
        case "show":
            return message.channel.send((await getContextByGuildId(guildId)).context.map(({content, role}) => `${role}: ${content}`).join("\n"));
        case "logs":
        case "log":
        ...
        case "image":
            ...
            if (number > 5) return message.channel.send("Please provide a number less than 5.");
